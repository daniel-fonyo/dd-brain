...
When a store appears in position 3 of a GenAI carousel, we can't answer _why_. The `combinedScore` that determined its rank was computed, used to sort, and thrown away. The only score that survives to Snowflake is the original ML prediction — not the signal that actually decided the order.

This isn't unique to GenAI. Across the homepage pipeline, ranking steps compute scores, apply weights, and choose strategies. Most of this work is invisible by the time data reaches Iguazu. If you want to log a new signal today, you need to:

1. Add a typed field to a data class
2. Manually copy it through 2-3 carrier objects
...
Signals recorded at any phase survive to Snowflake because the collector is a mutable object on the request context — same instance from start to finish.
...
- **Does not change StoreEntity or any entity class.** Signals bypass entities entirely — collector to adapter to proto.
